...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (profileError || !profile) {
      // Profile missing — still logged in, go to home
      router.push('/')
      router.refresh()
      return
    }
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      ...
      options: {
        ...
        shouldCreateUser: false, // only existing accounts can use magic link sign-in
      },
    })
      ...
      <div className="w-full max-w-4xl bg-white border border-gray-100 rounded-2xl overflow-hidden flex" style={{ minHeight: '560px' }}>
        ...
        <div className="flex-1 flex flex-col justify-center p-8 lg:p-10">
          ...
          <div className="flex items-center gap-3 mb-4 text-xs text-gray-400">
            <div className="flex-1 h-px bg-gray-100" />or sign in with email<div className="flex-1 h-px bg-gray-100" />
          </div>

          {/* Password / Magic link mode switch */}
          <div className="flex gap-1 bg-gray-50 rounded-xl p-1 mb-4">
            <button type="button" onClick={() => setMode('password')}
              className="flex-1 text-xs font-medium py-2 rounded-lg transition-colors"
              style={mode === 'password' ? { background: 'white', color: '#085041', boxShadow: '0 1px 2px rgba(0,0,0,0.06)' } : { color: '#9CA3AF' }}>
              Password
            </button>
            <button type="button" onClick={() => setMode('magic')}
              ...
              style={mode === 'magic' ? { background: 'white', color: '#085041', boxShadow: '0 1px 2px rgba(0,0,0,0.06)' } : { color: '#9CA3AF' }}>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="text-right">
                  <Link href="/auth/forgot-password" className="text-xs font-medium" style={{ color: '#0F6E56' }}>Forgot password?</Link>
                </div>
                ...
                <>
                  ...
                  <form onSubmit={handleMagicLink} className="space-y-3">
                    ...
                    <button type="submit" disabled={magicLoading}
                      ...
                      style={{ background: '#1D9E75' }}>
                      {magicLoading ? <><Loader2 size={14} className="animate-spin" /> Sending…</> : <><Sparkles size={14} /> Send magic link</>}
                    </button>
          ...
          <p className="text-center text-sm text-gray-500 mt-5">
            No account?{' '}
            <Link href="/auth/signup" className="font-semibold" style={{ color: '#0F6E56' }}>Create one free</Link>
          </p>
        </div>
      </div>
    </div>
