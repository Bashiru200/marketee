...
import {
  ...
  Package, BarChart3, MapPinned, Megaphone, Inbox, CreditCard,
} from 'lucide-react'
...
function getPlans(currentPlan: Business['plan']) {
  ...
  return [
    ...
    },
    {
      ...
      name: '🔵 Pro Store',
      subtitle: 'Build your full online storefront',
      monthlyPrice: 49,
      annualPrice: 39,
      highlight: false,
      current: plan === 'pro_store',
      features: [
...
function ProfileStrength({
  ...
  const color =
    profileScore >= 80 ? '#1D9E75' : profileScore >= 50 ? '#BA7517' : '#EF4444'

  const label =
    profileScore >= 80 ? 'Strong' : profileScore >= 50 ? 'Good' : 'Needs work'
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
      ...
      <p className="text-sm text-gray-500 mb-5">
        ...
        {requiredPlan === 'growth' ? 'Growth' : 'Pro Store'} plan.
      </p>
    </div>
  )
}
...
const NAV_ITEMS: { id: Tab; label: string; icon: React.ElementType }[] = [
  ...
  { id: 'listing', label: 'Edit Listing', icon: Edit },
    ...
    <>
      ...
      <aside
        className={`
          ...
          w-72 flex-shrink-0
          bg-white border-r lg:border border-gray-100 lg:rounded-3xl
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h2 className="font-bold text-gray-900 truncate max-w-[190px]">
              {businessName}
            </h2>
          </div>
        ...
        <div className="mb-6 rounded-2xl p-4" style={{ background: '#F0FAF6' }}>
          <p className="text-xs text-gray-500 mb-1">Current plan</p>
          <p className="font-bold text-gray-900">
            {plan === 'starter' ? 'Starter' : plan === 'growth' ? 'Growth' : 'Pro Store'}
          </p>
        </div>
        ...
        <div className="absolute bottom-5 left-5 right-5">
          <Link
            href="/"
            className="block text-center text-sm font-medium text-gray-500 hover:text-green-700"
          >
            ← Back to site
          </Link>
        </div>
...
export default function DashboardPage() {
  ...
  const [form, setForm] = useState({
    ...
    plan_interval: 'monthly' as Billing,
  })
  ...
  async function saveSlug() {
    ...
    if (!cleaned) {
      setSlugMsg({ text: 'Enter a valid store name', ok: false })
      return
    }

    if (normalizePlan(biz.plan) !== 'pro_store') {
      setSlugMsg({ text: 'Upgrade to Pro Store to use a custom URL', ok: false })
      return
    }
    ...
    if (existing) {
      setSlugMsg({ text: 'That URL is already taken — try another', ok: false })
      setSlugSaving(false)
      return
    }
    ...
    if (error) {
      setSlugMsg({ text: error.message, ok: false })
    } else {
    ...
    if (process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY) {
      try {
        const addr = encodeURIComponent(
          `${form.street ? `${form.street}, ` : ''}${form.city}, ${form.state}, USA`
        )

        const geoRes = await fetch(
          `https://maps.googleapis.com/maps/api/geocode/json?address=${addr}&key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`
        )
  ...
  const inputCls =
    'w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent transition-all'
  ...
  const profileScore = Math.round(
    (recommendations.filter((r) => r.done).length / recommendations.length) * 100
  )
      ...
      <div className="max-w-7xl mx-auto lg:flex lg:gap-6 px-4 py-6">
        ...
        <main className="flex-1 min-w-0">
          <div className="hidden lg:flex items-center justify-between mb-8">
            ...
            <Link
              href={`/businesses/${biz.id}`}
              className="flex items-center gap-2 text-sm font-medium text-white px-4 py-2 rounded-xl hover:opacity-90"
              style={{ background: '#1D9E75' }}
            >
              <Eye size={14} />
              View listing
                ...
                <div className="flex gap-4 p-5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-gray-500 line-clamp-2">
                      {biz.description || 'No description yet.'}
                    </p>

                    <p className="text-xs text-gray-400 mt-2">
                      📍{' '}
                      {[biz.street ?? biz.address, biz.city, biz.state, biz.zip]
                        .filter(Boolean)
                    ...
                    {biz.country && (
                      <p className="text-xs text-gray-400 mt-0.5">🌍 {biz.country}</p>
                    )}
                  </div>
                </div>
              </div>
                      ...
                      <div key={r.id} className="flex gap-3 p-3 bg-gray-50 rounded-xl">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-800">
                            {r.profiles?.name ?? 'Anonymous'} · {'⭐'.repeat(r.rating)}
                          </p>
          ...
          {tab === 'listing' && (
            ...
            >
              ...
              {saved && (
                <div className="px-4 py-3 rounded-xl text-sm text-green-700 bg-green-50 border border-green-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <ImageUpload
                  bucket="businesses"
                  folder={`${profile?.business_id ?? 'unknown'}/logo`}
              ...
              <div>
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    onChange={(e) => upd('hours_open', e.target.value)}
                  ...
                  <>
                    ...
                    {slugMsg && (
                      <p
                        ...
                        style={{ color: slugMsg.ok ? '#1D9E75' : '#DC2626' }}
                      >
                        {slugMsg.text}
                      </p>
                    )}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5">
                ...
                {reviews.length === 0 ? (
                  <p className="text-sm text-gray-400">
                    No reviews yet. Share your listing link to get your first review.
                  </p>
                ) : (
                  <div className="space-y-3">
                    {reviews.map((r) => (
          ...
          {tab === 'upgrade' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                >
                  <div
                    ...
                    style={{
                      transform: billing === 'annual' ? 'translateX(26px)' : 'translateX(2px)',
                    }}
                  />
                </button>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: plan.highlight ? '#9FE1CB' : '#6B7280' }}
                      >
                        {plan.subtitle}
                      </p>
                      ...
                      <div className="flex items-baseline gap-1 mb-1">
                        ...
                        {plan.monthlyPrice !== 0 && (
                          <span style={{ color: plan.highlight ? '#9FE1CB' : '#9CA3AF' }}>
                            /month
                          </span>
                        )}
                      </div>
                      ...
                      <ul className="space-y-2.5 mb-6 flex-1 mt-4">
                        {plan.features.map((f) => (
                          ...
                          >
                            <span style={{ color: plan.highlight ? '#9FE1CB' : '#1D9E75' }}>
                              ✓
                            </span>
                            {f}
                          </li>
              ...
              <div className="rounded-2xl border border-gray-100 p-5 bg-gray-50 text-center">
                ...
                <a
                  href="mailto:hello@markeetee.com"
