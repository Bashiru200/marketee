...
import PlatformAnalytics    from '@/components/admin/AdminManagement'
...
type ActiveTab = 'businesses' | 'users' | 'reviews' | 'analytics' | 'revenue' | 'audit' | 'admins' | 'announcements' | 'flags' | 'broadcast' | 'claims' | 'reports' | 'beta'
...
export default function AdminPage() {
  ...
  // ── Tab visibility — fixed: no more enabledFlags reference ────────────
  ...
  async function executeBulkAction() {
    ...
    if (bulkAction === 'premium') {
      ...
      await log({ action:'set_plan', entityType:'plan', entityName:`Bulk: ${ids.length}`, details:{ to:'premium', count: ids.length } })
    ...
    setSelected(new Set()); setBulkAction(''); setConfirmBulk(false); setBulkLoading(false)
  }
  ...
  async function setPlan(id: string, plan: Plan) {
    ...
    const { error } = await supabase.from('businesses').update({
      plan, premium: plan !== 'free', featured: plan === 'storefront',
    }).eq('id', id)
    if (error) { showToast(`Error: ${error.message}`) }
    else {
    ...
    setUpdating(null); setPlanModal(null)
  }
  ...
  async function deleteBusiness(id: string) {
    ...
    setConfirmDel(null); setDeleting(null)
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{isSuperAdmin ? 'Super admin · Full access' : 'Admin · Limited access'}</p>
          </div>
        </div>
        <Link href="/" className="text-sm text-gray-500 hover:text-green-700 transition-colors">← Back to site</Link>
      </div>
      ...
      <div className="flex gap-1 bg-white border border-gray-100 rounded-2xl p-1 mb-6 flex-wrap">
        {NAV_TABS.filter(t => canSeeTab(t)).map(({ id, label, icon:Icon }) => (
          <button key={id} onClick={() => setActiveTab(id)}
            ...
            style={activeTab === id ? { background:'#085041', color:'white' } : { color:'#6B7280' }}>
            <Icon size={15} /> {label}
          </button>
        ))}
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          ...
          {selected.size > 0 && (
            <div className="flex items-center gap-3 px-4 py-3 border-b border-amber-200" style={{ background:'#FFFBEB' }}>
              ...
              {bulkAction && (
                <button onClick={() => bulkAction === 'delete' ? setConfirmBulk(true) : executeBulkAction()}
                  ...
                  {bulkLoading ? <><Loader2 size={13} className="animate-spin" /> Running…</> : `Apply to ${selected.size}`}
                </button>
              )}
              <button onClick={() => setSelected(new Set())} className="text-xs text-amber-600 hover:text-amber-800 ml-auto">
                Clear selection
              </button>
            </div>
          )}
            ...
            <div className="flex gap-1 bg-gray-100 rounded-xl p-1 overflow-x-auto">
              {FILTER_TABS.map(t => (
                <button key={t.id} onClick={() => setFilterTab(t.id)}
                  ...
                  style={filterTab === t.id ? { background:'#1D9E75', color:'white' } : { color:'#6B7280' }}>
                  {t.label}
          ...
          {filtered.length > 0 && (
            <div className="flex items-center gap-3 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
              ...
              <span className="text-xs text-gray-500">
                {selected.size > 0 ? `${selected.size} of ${filtered.length} selected` : `Select all ${filtered.length} businesses`}
              </span>
            </div>
          )}
              ...
              {filtered.map(b => (
                <div key={b.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors"
                  ...
                  <input type="checkbox" checked={selected.has(b.id)} onChange={() => toggleSelect(b.id)}
                    onClick={e => e.stopPropagation()} className="rounded accent-green-600 cursor-pointer flex-shrink-0" />
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <Link href={`/businesses/${b.id}`} target="_blank" title="View listing"
                    ...
                    <button onClick={() => toggleVerified(b)} disabled={updating === b.id + 'verified'}
                      ...
                      {updating === b.id + 'verified' ? <Loader2 size={13} className="animate-spin" /> : <BadgeCheck size={14} />}
                    </button>
                    <button onClick={() => setPlanModal(b)} disabled={updating === b.id + 'plan'}
                      title="Manage plan"
                      className="w-8 h-8 flex items-center justify-center rounded-lg border transition-colors"
                      style={b.plan === 'storefront' ? { background:'#c5eadb', borderColor:'#085041', color:'#085041' }
                        : b.plan === 'premium' ? { background:'#FEF3C7', borderColor:'#F59E0B', color:'#D97706' }
                        : { borderColor:'#E5E7EB', color:'#9CA3AF' }}>
                      {updating === b.id + 'plan' ? <Loader2 size={13} className="animate-spin" /> : <Crown size={14} />}
                    </button>
                    {confirmDel === b.id ? (
                      <div className="flex items-center gap-1">
                        <button onClick={() => deleteBusiness(b.id)} disabled={deleting === b.id}
                          className="w-8 h-8 flex items-center justify-center rounded-lg bg-red-500 text-white hover:bg-red-600 transition-colors">
                          {deleting === b.id ? <Loader2 size={13} className="animate-spin" /> : <CheckCircle2 size={14} />}
                        </button>
                      ...
                      <button onClick={() => setConfirmDel(b.id)} title="Delete business"
          ...
          <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
            <p className="text-xs text-gray-400">Showing {filtered.length} of {businesses.length} businesses</p>
          </div>
        </div>
      )}
      ...
      {confirmBulk && bulkAction === 'delete' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background:'rgba(0,0,0,0.5)' }} onClick={() => setConfirmBulk(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 bg-red-50">
              <Trash2 size={22} className="text-red-500" />
            </div>
            <h3 className="text-lg font-bold text-gray-900 text-center mb-2">Delete {selected.size} businesses?</h3>
            <p className="text-sm text-gray-500 text-center mb-6">
              This will permanently delete <strong>{selected.size} businesses</strong> and all their data. This cannot be undone.
            </p>
      ...
      {planModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background:'rgba(0,0,0,0.5)', backdropFilter:'blur(4px)' }} onClick={() => setPlanModal(null)}>
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden"
            style={{ boxShadow:'0 24px 80px rgba(0,0,0,0.25)' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-3">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-4">Set subscription plan</p>
              {(['free','premium','storefront'] as Plan[]).map(plan => {
                const cfg     = PLAN_CONFIG[plan]
                const current = (planModal.plan ?? 'free') === plan
                return (
                  <button key={plan} onClick={() => setPlan(planModal.id, plan)}
                    ...
                    style={{ borderColor:current ? cfg.color : '#E5E7EB', background:current ? cfg.bg : '#FAFAFA', opacity: updating === planModal.id + 'plan' ? 0.6 : 1 }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          ...
                          : plan === 'premium'   ? '$40/mo · Products, featured, cover photo'
                          :                        '$60/mo · Custom URL, analytics, verified badge'}
                        </p>
                      </div>
                    </div>
                    {current ? (
                      <span className="text-xs font-medium px-2 py-1 rounded-full text-white" style={{ background:cfg.color }}>Active</span>
