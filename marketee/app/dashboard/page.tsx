...
import {
  Eye, Star, MessageSquare, MapPin, Edit, TrendingUp,
...
interface Business {
  ...
  logo_url: string | null; hours_open: string | null; days_open: string[] | null
  rating: number; review_count: number; verified: boolean; premium: boolean; featured: boolean
  tags: string[] | null
}
...
function getRecommendations(
  ...
  tips.push({
    icon: '🖼️', title: 'Add a cover photo',
    body: 'Businesses with cover photos get 3x more profile views.',
    done: !!biz.cover_image,
  })
  tips.push({
    icon: '📝', title: 'Write a description',
    body: 'Tell customers what makes your business special and what you sell.',
    done: !!(biz.description && biz.description.length > 50),
  })
  tips.push({
    icon: '📦', title: 'Add at least 3 products',
    body: 'Listings with products get 2x more customer enquiries.',
    done: products.length >= 3,
  })
  tips.push({
    icon: '🕐', title: 'Set your opening hours',
    body: "Customers want to know when you're open before visiting.",
    done: !!(biz.hours_open && biz.days_open && biz.days_open.length > 0),
  })
  tips.push({
    icon: '📍', title: 'Add your full address',
    body: 'A complete address helps customers find you on the map.',
    done: !!(biz.street && biz.city && biz.state),
  })
  tips.push({
    icon: '⭐', title: 'Get your first 5 reviews',
    body: 'Share your listing link with happy customers to build trust.',
    done: reviews.length >= 5,
  })
  tips.push({
    icon: '📱', title: 'Add your phone number',
    body: 'Make it easy for customers to call or WhatsApp you.',
    done: !!biz.phone,
  })
...
// ── Profile strength compact dropdown ────────────────────────────────────────
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Set avatar from persisted profile
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (saveError) { setError(saveError.message) }
      ...
      setBiz(b => b ? { ...b, ...form, cover_image: form.cover_image || null, logo_url: form.logo_url || null } : b)
      setSaved(true); setTimeout(() => setSaved(false), 3000)
    }
    setSaving(false)
  }

  // ── Avatar upload — save to profile ─────────────────────────────────────
  ...
  if (authLoading || loading) return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="hidden sm:block h-9 w-32 bg-gray-100 rounded-xl" style={{ animation: 'pulse 2s infinite' }} />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6">
        <div className="flex items-center gap-4">
          <div className="w-5 h-5 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
          <div className="flex-1">
            <div className="flex justify-between mb-2">
              <div className="h-4 w-32 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
              <div className="h-4 w-16 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
            </div>
            <div className="h-1.5 w-full bg-gray-100 rounded-full" style={{ animation: 'pulse 2s infinite' }} />
          </div>
          <div className="w-4 h-4 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-6">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="h-5 w-24 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
          <div className="h-5 w-12 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
        </div>
        <div className="flex gap-4 p-5">
          <div className="w-20 h-20 rounded-xl bg-gray-100 flex-shrink-0" style={{ animation: 'pulse 2s infinite' }} />
          <div className="flex-1 space-y-2">
            <div className="h-5 w-40 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
            ...
            <div className="h-3 w-48 bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <div className="h-5 w-32 bg-gray-100 rounded mb-4" style={{ animation: 'pulse 2s infinite' }} />
        <div className="space-y-3">
          {[1,2].map(i => (
            <div key={i} className="flex gap-3 p-3 bg-gray-50 rounded-xl">
              <div className="w-8 h-8 rounded-full bg-gray-200 flex-shrink-0" style={{ animation: 'pulse 2s infinite' }} />
              <div className="flex-1 space-y-1.5">
                <div className="h-3.5 w-32 bg-gray-200 rounded" style={{ animation: 'pulse 2s infinite' }} />
                <div className="h-3 w-full bg-gray-100 rounded" style={{ animation: 'pulse 2s infinite' }} />
              </div>
            </div>
          ))}
  ...
  const completedCount  = recommendations.filter(r => r.done).length
  const profileScore    = Math.round((completedCount / recommendations.length) * 100)
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href={`/businesses/${biz.id}`}
          className="hidden sm:flex items-center gap-2 text-sm font-medium text-white px-4 py-2 rounded-xl hover:opacity-90 transition-opacity"
      ...
      {tab==='overview' && (
        <div className="space-y-6">
          ...
          {/* Profile completeness — compact with dropdown */}
            ...
            <div className="flex gap-4 p-5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-2">
                  📍 {[biz.street, biz.city, biz.state].filter(Boolean).join(', ') || 'No location set'}
                  {biz.rating>0 && ` · ⭐ ${biz.rating.toFixed(1)}`}
                </p>
                {biz.hours_open && biz.days_open && biz.days_open.length>0 && (
                  <p className="text-xs text-gray-400 mt-1">
                    🕐 {biz.hours_open} · {biz.days_open.slice(0,3).join(', ')}{biz.days_open.length>3 ? '…' : ''}
                  </p>
                )}
              </div>
            </div>
      ...
      {tab==='listing' && (
        <form onSubmit={handleSave} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {([
                ['Business name', 'name',    'text',  'Mama Titi African Kitchen'],
                ['Phone',         'phone',   'tel',   '+1 (713) 555-0192'],
                ...
                <div key={key}>
                  ...
                  <input type={type} value={form[key as keyof typeof form] as string} onChange={e => upd(key, e.target.value)}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="sm:col-span-3">
                <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Street address</label>
                <input value={form.street} onChange={e => upd('street', e.target.value)}
                  placeholder="4821 Main St"
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent" />
              </div>
              <div className="sm:col-span-2">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">State</label>
          ...
          <div>
            ...
            <div className="mb-3">
              <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Hours (e.g. 9am – 9pm)</label>
              <input value={form.hours_open} onChange={e => upd('hours_open', e.target.value)}
                placeholder="9:00 AM – 9:00 PM"
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide flex items-center gap-1.5">
                <Calendar size={13} /> Days open
              </label>
                ...
                {DAYS.map(day => {
                  ...
                  return (
                    <button key={day} type="button" onClick={() => toggleDay(day)}
                      ...
                      style={{
                        background:  isOn ? '#1D9E75' : 'transparent',
                        borderColor: isOn ? '#1D9E75' : '#E5E7EB',
          ...
          {showAddProduct && (
            <form onSubmit={handleAddProduct} className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
              ...
              <ImageUpload bucket="products" folder={profile?.business_id ?? 'unknown'}
                currentUrl={newProduct.image_url||null}
                onUpload={url => updProduct('image_url', url)}
                onRemove={() => updProduct('image_url', '')}
                label="Product image" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Product name *</label>
                  <input value={newProduct.name} onChange={e => updProduct('name', e.target.value)} required
                    placeholder="Jollof Rice (Party Size)"
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Price ($)</label>
