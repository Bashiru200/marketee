...
function textPreview(html: string, maxLength: number = 120) {
  return escapeHtml(html.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim().slice(0, maxLength))
}
  ...
  <tr><td style="padding:36px 32px;">
    ...
    <div style="margin-top:28px;background:#ffffff;border:1px solid #DCE8E3;border-radius:16px;padding:20px;">
      ...
      <div style="padding:12px 0;${i < arr.length - 1 ? 'border-bottom:1px solid #EDF1EF;' : ''}">
        ...
        <p style="margin:0 0 6px;font-size:14px;line-height:1.6;color:#2F3936;">${escapeHtml(textPreview(r.body, 140))}</p>
...
// Used by SendEmailModal — rich message from a business to a customer.
// Supports markdown-lite (bold, italic, bullets), image gallery, and up to 3
// link buttons.
// ═══════════════════════════════════════════════════════════════════════════
export function businessMessageTemplate({
        ...
        <tr>
          ${images.slice(0, 3).map(img => `
            <td width="${100 / Math.min(images.length, 3)}%" style="padding:2px;">
              <img src="${safeUrl(img)}" alt="" style="display:block;width:100%;height:auto;border-radius:8px;" />
            </td>
          `).join('')}
        </tr>
        ${images.length > 3 ? `
  ...
  // Link buttons — first is primary, others secondary
